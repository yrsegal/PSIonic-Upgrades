...
class ItemUnstableBattery(name: String) : ItemComponent(name) {
    ...
    override fun addHiddenTooltip(stack: ItemStack, playerIn: EntityPlayer, tooltip: MutableList<String>, advanced: Boolean) {
        ...
        TooltipHelper.addToTooltip(tooltip, " " + TextFormatting.AQUA +
                TooltipHelper.local("${LibMisc.MOD_ID}.cadstat.extra")
                + TextFormatting.GRAY + ": " + TooltipHelper.local("${LibMisc.MOD_ID}.upsides.boost_regen"))
        TooltipHelper.addToTooltip(tooltip, " " + TextFormatting.RED +
                TooltipHelper.local("${LibMisc.MOD_ID}.cadstat.downside")
                + TextFormatting.GRAY + ": " + TooltipHelper.local("${LibMisc.MOD_ID}.downsides.on_damage"))
    }
