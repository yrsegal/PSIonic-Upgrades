...
class ContainerCADMagazine(player: EntityPlayer, val stack: ItemStack) : Container() {
    val inventory = InventorySocketable(stack)
    ...
    init {
        ...
        for (slot in innerSlots) {
            addSlotToContainer(SlotBullet(inventory, i, slot.first, slot.second, i, true))
            i++
        }
        ...
        for (slot in outerSlots) {
            addSlotToContainer(SlotBullet(cadInventory, i, slot.first, slot.second, i, false))
            i++
        }
    }

    class SlotBullet(val socketable: InventorySocketable, index: Int, xPosition: Int, yPosition: Int, var socketSlot: Int, val dark: Boolean) : Slot(socketable, index, xPosition, yPosition) {
        ...
        override fun isItemValid(stack: ItemStack?): Boolean {
            if (stack!!.item is ISpellContainer) {
                ...
                if (container.containsSpell(stack) && isSlotEnabled()) {
                    return socketable.isItemValidForSlot(socketSlot, stack)
                }
            }
